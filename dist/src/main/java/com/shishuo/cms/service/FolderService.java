...
public class FolderService {
	...
	public void updateFolderById(long folderId, String name, String ename,
			FolderConstant.status status, String content,String summary, int height, int width,int isMainSeries,MultipartFile multipartFile,MultipartFile mainFile) throws IOException, FolderNotFoundException {
		Folder folder = this.getFolderById(folderId);
		String picture = folder.getPicture();
		String mainPicture = folder.getMainPicture();
		if (multipartFile != null && !multipartFile.isEmpty()) {
			picture = MediaUtils.saveImage(multipartFile,0,0);
		}
		if (mainFile != null && !mainFile.isEmpty()) {
			mainPicture = MediaUtils.saveImage(mainFile,0,0);
		}
		folderDao.updateFolderById(folderId, name, ename, status, content,summary,
				height, width,isMainSeries,picture,mainPicture);
	}
